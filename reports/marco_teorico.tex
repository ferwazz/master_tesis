...
Por último, la relación masa-radio, por ejemplo $R_{*}=kM_{*}^{x}$ ($x\sim 0.8$ para estrellas de secuencia principal del tipo espectral F-K), y k una constante cuyo valor depende del tipo estelar de la estrella y su edad. \cite{allen2001allen}
...
\begin{figure}[h!]
  \centering
   ...
   \caption{Fig. 2.3. Curva de luz de XO-3, tomada con el filtro R de Johnson en el telescopio de $84~cm$ en el Observatorio Astronómico Nacional (OAN-SPM) el 21 de octubre del 2016.}
    \label{fig_2_3_XO3}
\end{figure}
...
En la figura 2.6 podemos apreciar una curva de luz de alta cadencia y su respectivo espectro de frecuencias, como podemos observar, se aprecia una importante componente de bajas frecuencias debido a las tendencias en la curva de luz, provocadas por variables atmosféricas. Es por eso que en este trabajo se modeló el ruido de una manera en la que el ruido simulado tenga componentes que observamos en curvas de luz reales \textcolor{red}{(véase SECCION SIM RUIDO)}, a diferencia de simplemente añadir un ruido con distribución gaussiana (ruido blanco) el cual tiene un espectro de frecuencias plano.
...
Para toda matriz $A$ con dimensiones $m\times n$, su descomposición en valores singulares ó SVD (\textit{del inglés Singular Value Decomposition}) es:
