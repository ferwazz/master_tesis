...
Debido al corto tiempo de exposición, el número de cuentas por estrella es pequeño, por lo cual, el ruido electr\'onico se vuelve un factor importante. Por otro lado, las perturbaciones atmosf\'ericas afectan el \textit{seeing} que es una medida de la estabilidad de la atm\'osfera. Esto afecta a las im\'agenes de alta cadencia de dos maneras: 1) cambiando de posici\'on la estrella en el detector o deform\'andola, lo cual se convierte en una fuerte contribici\'on al ruido cuando se lleva a cabo la fotomtr\'ia de apertura \textcolor{red}{Aquí también puede ir nuestro artículo de referencia}. El \textit{seeing} durante las temporadas fue bueno \textcolor{red}{NO se si esta oración sea correcta, pues no hicimos ninguna evaluación del seeing}, aunque se presentan ligeras variaciones en la posición de la estrella de toma a toma, este es el motivo por el cual no vemos la estrella como una circunferencia perfecta. Este ruido se ve reflejado a la hora de obtener la curva de luz de las imágenes. La cantidad de ruido se puede cuantificar calculando la SNR (2.11), la cual está en función de la magnitud aparente de la estrella.
...
Esta misma distribución se encontró en diferentes curvas de luz, por lo que se ajustó un modelo de ruido con dos componentes gaussianas (3.2) a las distribuciones temporales de las curvas de luz \textcolor{red}{La ecuación se ve desacomodada en el PDF}. 
...
Ajustando $m(t)$ a todas las curvas de luz, se encontraron restricciones para el momento en que estemos modelando el ruido (véase III.2.3).\textcolor{red}{Qué quisiste decir aquí?? puedes ampliarlo?? } 
...
\begin{figure}[H]
  \centering
   ...
   \caption{Arriba: se muestra una curva de ruido blanco con dos componentes $G_{1}$ y $G_{2}$. Abajo: el espectro de frecuencias de la curva de luz, el cual permanece blanco sin importar el número de componentes gaussianas\textcolor{red}{no entendí la explicación sobre el espectro}.}
...
Para añadir estas contribuciones de baja frecuencia, se modeló una envolvente exponencial de la forma \textcolor{red}{la ecuación se ve rara en el PDF}:

\begin{equation}
  \displaystyle E(f)= C + a_{1}e^{-\dfrac{f}{b_{1}}} + a_{2}e^{-\dfrac{f}{b_{2}}};
\end{equation}

la función envolvente (3.19)\textcolor{red}{No encuentro esta referencia, deberías usar las referencias cruzadas de latex...}, está compuesta por dos componentes que decaen de manera exponencial hacia las altas frecuencias. Estas componentes son provocadas por fenómenos de baja frecuencia en la curva de luz, como tendencias o un posible tránsito. La envolvente tiene una asíntota horizontal en $C$, que representa el valor mínimo del espectro de frecuencias observado.

Utilizando esta envolvente, se altera el espectro de frecuencias de acuerdo a lo analizado en las curvas de luz de las observaciones. De manera similar a la metodología implementada para las distribuciones temporales\textcolor{red}{cuáles distribuciones temporales??}, se calcularon los parámetros de $E(f)$ para todas las curvas de luz y se obtuvieron restricciones para la posterior simulación de ruido sintético.

\textcolor{red}{otra vez, por favor explica la ecuacion, describe cada factor}
\begin{figure}[H]
  \centering
...
\textcolor{red}{según entiendo, tu simulación del ruido se basa en dos gaussianas y una envolvente exponencial negativa... es importante explicarlo así para una mejor comprensión en la lectura... }El resultado es una serie de tiempo $R(t)$ normalizada, de longitud $N$ con distribución $m(t)$. Los valores para la amplitud $(A_{1},A_{2})$, las medias   $(\mu_{1},\mu_{2})$ y la desviaciones $(\sigma_{1},\sigma_{2})$ de la función de distribución $m(t)$ \textcolor{red}{y los factores $a_1,b_1,a_2,b_2$ aunque estos son de la envolvente}, pueden ser elegidos de manera arbitraria\textcolor{red}{pero dentro de un rango específico donde caben todas nuestras observaciones, auqnue no se si este rango solo lo obtuviste para las envolventes. Esto es MUYYY importante}, dado que podemos cambiar la SNR de la serie de tiempo simplemente con: $R(t)*\sigma$. Para una curva normalizada $(\mu=1)$ la SNR es igual a $\sigma^{-1}$. Con esto podemos elegir la SNR que se necesite.
...
Utilizando esta metodología, se creó una base de datos de ruidos simulados para asegurar reproducibilidad y posteriormente hacer estadística. Se variaron los parámetros $(C,a_{1},b_{1},a_{2},b_{2})$ de manera uniforme dentro de los intervalos que comprenden los valores de cada curva de luz en los datos reales:
...
En este capítulo se plantea la metodología seguida para procesar los datos observacionales resultado de las múltiples temporadas en el OAN-SPM, \textcolor{red}{así como las simulaciones}. De los 8 objetos descritos en la sección III.1.2, se obtuvieron un total de 32 curvas de luz de alta cadencia (20 fps) y con una duración de aproximadamente 2 horas cada una. Esto con el propósito de igualar las condiciones que se tendrán en TAOS II.

\subsection*{III.4.1 Fotometría con APPHi}
\addcontentsline{toc}{subsection}{III.4.1.1 Fotometría con APPHi}

\textcolor{red}{ESTA SECCIÓN DEBERÍA IR AL PRINCIPIO DE LA METODOLOGÍA PORQUE REPRESENTA EL PUNTO DE PARTIDA PARA EL RESTO DEL ANÁLSIS, ES DECIR, LA OBTENCIÓN DE LAS CURVAS DE LUZ}
...
\begin{figure}[H]
  \centering
   ...
   \caption{Curva de luz de WASP-74 tomada el día 28 de mayo del 2017. En azul se muestra la curva de luz original obtenida mediante fotometría de apertura. En naranja la curva resultado del promedio móvil, y el verde la curva resultado de tomar la mediana en lugar del promedio \textcolor{red}{Yo veo azul rojo y negro, Que pasa??}. En el recuadro se muestra la SNR calculada para cada curva de utilizando (2.11).  }
...
Después multiplicamos el filtro, por el espectro de frecuencias para obtener el espectro filtrado $\mathcal{F}_{fil}(f)$\textcolor{red}{ No le vas a poner número a la ecuación??}:
...
\begin{figure}[h!]
  \centering
   ...
   \caption{Curva de luz de WASP-74 tomada el día 28 de mayo del 2017. En azul se muestra la curva de luz original obtenida mediante fotometría de apertura. En naranja \textcolor{red}{No es rojo} la curva resultado del filtrado de Fourier, utilizando una frecuencia de corte $FC=2x10^{-3}$.}
