...
Hay que destacar que TAOS-II, tendrá exposiciones de dos horas por campo, esto para observar con la menor masa de aire posible. Debido a esto será muy difícil observar un tránsito completo, se espera observar la entrada o la salida del exoplaneta frente al disco solar. Por este motivo, los análisis y la metodología presentadas en este trabajo, se aplicaron a curvas de luz de 2 horas.
...
\begin{equation}
  E(f)= C + a_{1}e^{-\dfrac{f}{b_{1}}} + a_{2}e^{-\dfrac{f}{b_{2}}};
\end{equation}
...
\begin{equation}
  F_{prom}(t)=  \dfrac{\sum_{i=m}^{m+n} F(t_{i})}{n}\hspace{1cm}m=0,n,2n,...,N-n;
\end{equation}
...
\begin{equation}
  F_{mov}(t)=  \dfrac{\sum_{i=m}^{m+n} F(t_{i})}{n}\hspace{1cm}m=0,1,2,...,N-n;
\end{equation}
