...
\addcontentsline{toc}{section}{IV.1 Identificación y eliminación de ruido en curvas de luz}
...
\addcontentsline{toc}{subsection}{IV.1.2 Datos observacionales}
...
\addcontentsline{toc}{subsubsection}{IV.1.2.1 Resultados del filtro de Fourier}

En la figura \ref{fig_4_2_transito_fou} podemos apreciar el resultado del filtro pasabajas de Fourier, aplicado a la curva de luz de HAT-P-37b. A pesar de ser una curva con mucho ruido, se puede apreciar a simple vista la forma del tránsito. Se utilizó una frecuencia de corte $FC$ de $2x10^{-3}$. También se muestra el modelo teórico del tránsito en negro, simulado usando los parámetros presentados en \cite{bakos2012hat} y \textit{BATMAN} (véase III.3.1). La similitud entre el modelo y la curva filtrada es alto, el coeficiente de correlación es de $C=0.982$.
...
\begin{figure}[h!]
	\centering
	 ...
	 \caption{Azul: curva de luz de HAT-P-37b después del filtrado de altas frecuencias. La señal a ruido resultante fue de $SNR=124.75$. La línea negra representa el modelo teórico, utilizando los parámetros presentados en \cite{bakos2012hat}. El coeficiente de correlación entre la curva y el modelo es $C=0.982$.}
	  \label{fig_4_2_transito_fou}
  \end{figure}

\subsubsection*{IV.1.2.2 Resultados del promedio móvil}
\addcontentsline{toc}{subsubsection}{IV.1.2.2 Resultados del promedio móvil}

En la figura \ref{fig_4_3_transito_mov} podemos apreciar el resultado del promedio móvil, aplicado a la curva de luz de HAT-P-37b. Para este resultado, se utilizó una ventana de 3000 frames. De igual manera se muestra en negro el modelo teórico. La similitud entre el modelo y la curva filtrada es alto, el coeficiente de correlación es de $C=0.962$.
...
\centering
	...
	\label{fig_4_3_transito_mov}
\end{figure}

Como se observa en la figura \ref{fig_4_3_transito_mov}, este método altera la pendiente de la curva, donde el planeta está saliendo de la superficie solar. Esta deformación es proporcional a la longitud de la ventana de promediado, por lo que ventanas pequeñas (1000-3000 frames) pueden dar mejores coeficientes de correlación. 


\subsubsection*{IV.1.2.3 Resultados de PCA}
\addcontentsline{toc}{subsubsection}{IV.1.2.3 Resultados del PCA}

En la figura \ref{fig_4_4_transito_pca} podemos apreciar el resultado del filtrado utilizando el análisis de componentes principales (PCA), aplicado a la curva de luz de HAT-P-37b. Se calcularon las componentes principales de un conjunto de 25 tránsitos simulados con diferentes $\Delta F$ y $t_{c}$. Debido a que la componentes principales son calculadas utilizando una variedad de parámetros, la curva resultado solo posee la forma de tránsito, sin embargo, no es ideal para ser utilizada para el cálculo de observables.
...
\begin{figure}[h!]
	\centering
		...
		\label{fig_4_4_transito_pca}
\end{figure}

Esta metodología funciona para la visualización, y como método de comparación entre los modelos teóricos y las curvas de luz. Si las curva de luz, contiene componentes principales similares a las del modelo, se producirá una curva similar a la que se observa en la figura \ref{fig_4_4_transito_pca}. Si la curva es solo ruido, el resultado será una curva errática sin ningún parecido con el modelo.\textcolor{red}{HAY QUE PRESENTAR UN EJEMPLO DE ESTO}


\subsubsection*{IV.1.2.4 Resumen de resultados con datos observacionales}
\addcontentsline{toc}{subsubsection}{IV.1.2.4 Resumen de resultados con datos observacionales}

En la tabla \ref{tab_4_1_resultados} se presentan todos los resultados de la mejora de la SNR utilizando las metodologías descritas anteriormente.
...
\subsection*{IV.1.3 Curvas de luz simuladas}
\addcontentsline{toc}{subsection}{IV.1.3 Curvas de luz simuladas}
...
\addcontentsline{toc}{section}{IV.2 Determinación de candidatos a tránsitos}
...
\addcontentsline{toc}{subsection}{IV.2.1 Datos observacionales}
...
\addcontentsline{toc}{subsection}{IV.2.2 Simulaciones}
...
\addcontentsline{toc}{subsubsection}{IV.2.2.1 Correlaciones resultantes del filtrado de Fourier}
...
\begin{figure}[h!]
	\centering
		\includegraphics[max size={0.9\textwidth}{0.9\textheight}]{./figures/corrFourierSNR_10.png}
		...
		\label{fig_4_5_correlaciones_fou_10}
\end{figure}

\begin{figure}[h!]
	\centering
		\includegraphics[max size={0.9\textwidth}{0.9\textheight}]{./figures/corrFourierSNR_100.png}
		...
		\label{fig_4_6_correlaciones_fou_100}
\end{figure}
...
\addcontentsline{toc}{subsubsection}{IV.2.2.2 Correlaciones resultantes del promedio móvil}
...
\begin{figure}[h!]
	\centering
		...
		\label{fig_4_7_correlaciones_mov_10}
\end{figure}
...
\begin{figure}[h!]
	\centering
		...
		\label{fig_4_8_correlaciones_mov_100}
\end{figure}

Para el método de promedio móvil, la cota inferior para determinar una curva como candidata a tránsito es de $C \geq 0.84 $.

Si analizamos la figura \ref{fig_4_8_correlaciones_mov_100}, vemos que para este método de filtrado, los valores de $C$ están en función de $\Delta F$ y más notablemente de $t_{c}$. Esto se debe a lo que se mencionaba anteriormente (véase IV.1.2.2), este método altera la forma de la señal inmersa en el ruido. Esta deformación aumenta de manera proporcional a $t_{c}$.


\subsubsection*{IV.2.2.3 Correlaciones resultantes de PCA}
\addcontentsline{toc}{subsubsection}{IV.2.2.3 Correlaciones resultantes de PCA}

De manera análoga, las figuras \ref{fig_4_9_correlaciones_pca_10} y \ref{fig_4_10_correlaciones_pca_100} muestran la correlación, entre el modelo y la curva filtrada con el método de análisis de componentes principales (PCA), para curvas con $SNR=10$ y $SNR=100$ respectivamente. El color nos indica la media de los coeficientes de correlación ($C$), de los 100 ruidos con diferentes parámetros (véase III.2.3).

\begin{figure}[h!]
	\centering
		\includegraphics[max size={0.9\textwidth}{0.9\textheight}]{./figures/corrPCASNR_10.png}
		...
		\label{fig_4_9_correlaciones_pca_10}
\end{figure}

\begin{figure}[h!]
	\centering
		\includegraphics[max size={0.9\textwidth}{0.9\textheight}]{./figures/corrPCASNR_100.png}
		...
		\label{fig_4_10_correlaciones_pca_100}
\end{figure}
...
Si analizamos las figuras  \ref{fig_4_9_correlaciones_pca_10} y \ref{fig_4_10_correlaciones_pca_100}, vemos que al igual que para el método de Fourier, no se ve una correlación significativa entre $C$ y $t_{c}$. 